...
class AxisSignalGeneratorAMOV3(SocIp):
    bindto = ['user.org:user:axis_signal_gen_amo_v3:1.0']
    ...
    def sweep_config(self, config, debug = False, plot = False):
        ...
        # Check if amp is defined.
        if 'amp' not in config.keys():
            config['amp'] = [0.99, 0.99] # Default max, no amplitude modulation.
        ...
        if (plot):
            ...
            plt.figure(dpi=100);
            plt.plot(xx*self.cfg['SWEEP_TIME_US'],yy,'-b',x*self.cfg['SWEEP_TIME_US'],y,'*r');
            plt.xlabel("t [us]");
            plt.ylabel("A [Norm]");
    ...
    def sweep_config_write(self, config, debug = False):
        ...
        buff[11] = config['regs']['poff_reg']
        buff[12] = 0 # TODO: ctrl not used yet.
    ...
    def sweep_config_write_m(self, config, debug = False):
        ...
        for i in np.arange(Ncfg):
            ...
            buff[i*(self.NREG+1) + 11] = cfg['regs']['poff_reg']
            buff[i*(self.NREG+1) + 12] = 0
    ...
    def set_single(self, ch = 0, addr = 0, f = 0, g = 0, debug = False):
        ...
        amod_c0_reg = int(round(g*(2**(self.BAMP-1)))) # Q1.x format.
        amod_c1_reg = 0
        poff_reg    = 0
        ctrl_reg    = 0
...
class QickAmoSoc(QickSoc):
    ...
    def map_signal_paths(self):
        ...
        for key, val in self.ip_dict.items():
            if val['driver'] == AxisSignalGeneratorAMOV3:
    ...
    def description(self):
        ...
        for iGen, gen in enumerate(self.gens):
            if gen['type'] == "axis_signal_gen_amo_v3":
