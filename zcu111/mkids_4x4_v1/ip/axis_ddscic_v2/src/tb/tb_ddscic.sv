...
reg					sync_demux;
wire [NCH*80-1:0]	dout_demux;
wire			    valid_demux;

wire [39:0]			dout_real_ii [NCH];
wire [39:0]			dout_imag_ii [NCH];

genvar i;
generate
	for (i=0; i<NCH; i=i+1) begin
		assign dout_real_ii[i] = dout_demux[2*i*40 +: 40];
		assign dout_imag_ii[i] = dout_demux[(2*i+1)*40 +: 40];
	end
endgenerate
