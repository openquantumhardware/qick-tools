module ddscic
	...
	(
		...
		// Registers.
		input	wire		DDS_SYNC_REG	,
		input	wire [1:0]	DDS_OUTSEL_REG	,
...
dds_top
	...
	(
		...
		// Output data.
		.dout		(dds_dout		),
		.dout_last	(dds_dout_last	),

		// Registers.
		.SYNC_REG	(DDS_SYNC_REG	),
		.OUTSEL_REG	(DDS_OUTSEL_REG	)
	);

// CIC input data.
assign cic_din_real = {{(BCIC-B){dds_dout[15]}},dds_dout[15:0]};
assign cic_din_imag = {{(BCIC-B){dds_dout[31]}},dds_dout[31:16]};
